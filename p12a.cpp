/* P12a - File archiver and compressor.  Matt Mahoney
Like P6, but uses 1 and 2 word contexts, and a faster
hash function.

P12a is identical to P12 in both archive compatibility and performance,
but includes minor ports to support not only Borland (bcc32 5.02)
but also DJGPP (gxx 2.952, Windows 98, compile using -O2).
It does not use std.h, so gives bogus warnings when compiling with Borland.
...
Sigma2::Sigma2(double RL) {  
  for (int c0=0; c0<256; ++c0)
    for (int c1=0; c1<256; ++c1)
      table[(c0<<8)+c1]=short(1024*RL*(c0+c1+1)/((c0+0.5)*(c1+0.5)));
}
...
void Predictor::update(const int y) {
  ...
  static const int RS=int(1024*0.06);
  const int CMAX=250;
  for (int i=0; i<NX; ++i) {
    Wt& w=wt[x[i]+s0];
